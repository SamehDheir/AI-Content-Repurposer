...
export function ContentViewer({ job, onClose, onJobUpdate }: Props) {
  ...
  return (
    ...
    >
      <div
        className={"relative w-full max-w-2xl max-h-[90vh] md:max-h-[88vh] flex flex-col rounded-2xl overflow-hidden shadow-2xl bg-white border-gray-200 dark:bg-[#141416] dark:border-white/8"}
      ...
      >
        ...
        {/* ── Header ── */}
        <div className={"flex items-center justify-between px-4 md:px-5 py-3 md:py-4 border-b border-gray-200 dark:border-white/8"}>
          ...
          <button
            onClick={onClose}
            className={"w-8 h-8 md:w-7 md:h-7 rounded-lg flex items-center justify-center transition-all text-gray-500 hover:text-gray-700 hover:bg-gray-100 dark:text-zinc-500 dark:hover:text-zinc-300 dark:hover:bg-white/10"}
            aria-label="Close"
          >
            <X size={15} />
          </button>
        ...
        {(localImageUrl || job.imageUrl || generatingImage) && (
          <div className={"px-5 py-3 border-b border-gray-200 dark:border-white/8 bg-zinc-900/50"}>
            ...
            {generatingImage ? (
              ...
              <div 
                className={`rounded-lg overflow-hidden border border-white/10 mx-auto transition-all ${ imageZoomed ? "max-h-96 w-full" : "max-h-32 w-48" }`}
              >
                <img
        ...
        {/* ── Tabs ── */}
        <div className={"flex items-center gap-1 px-4 md:px-5 pt-3 pb-0 border-b border-gray-200 dark:border-white/8"}>
          {TABS.map((t) => {
            ...
            return (
              <button
                ...
                className={`relative flex items-center gap-1.5 px-2 md:px-3 py-2 -mb-px text-xs font-medium transition-all rounded-t-lg ${
                  isActive
                    ? `text-gray-900 bg-gray-100 border-gray-300 dark:text-zinc-100 dark:bg-white/8 dark:border-white/10 border-b-white dark:border-b-[#141416]`
                    : hasContent
                    ? `text-gray-500 hover:text-gray-700 hover:bg-gray-100 dark:text-zinc-500 dark:hover:text-zinc-300 dark:hover:bg-white/5`
                    : `text-gray-400 dark:text-zinc-700 cursor-not-allowed`
                }`}
        ...
        {/* ── Content area ── */}
        <div className={"flex-1 overflow-y-auto px-4 md:px-5 py-4 md:py-5 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-gray-300 hover:scrollbar-thumb-gray-400 dark:scrollbar-thumb-white/10 dark:hover:scrollbar-thumb-white/20"}>
          {!body ? (
            <div className="flex items-center justify-center h-32">
              <p className={"text-sm text-gray-500 dark:text-zinc-600"}>No content available.</p>
            </div>
          ) : (
            <>
        ...
        {/* ── Footer ── */}
        <div className={"flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-0 px-4 md:px-5 py-3 md:py-3.5 border-t border-gray-200 bg-gray-50 dark:border-white/8 dark:bg-white/3"}>
          <div className="flex items-center gap-1.5">
            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500/60" />
            <span className={"text-xs text-gray-500 dark:text-zinc-600"}>
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            {!localImageUrl && !job.imageUrl && (
              <button
                ...
                className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold transition-all duration-200 ${ generatingImage ? "bg-zinc-700 text-zinc-400 cursor-not-allowed" : "bg-purple-600 hover:bg-purple-500 text-white shadow-lg shadow-purple-500/20" } min-h-[44px]`}
            ...
            <button
              onClick={handleCopy}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold transition-all duration-200 ${ copied ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/30" : "bg-indigo-600 hover:bg-indigo-500 text-white shadow-lg shadow-indigo-500/20" } min-h-[44px]`}
