...
2. `JobsProcessor` ([jobs.processor.ts](ai-repurposer-backend/src/modules/jobs/jobs.processor.ts)) — the single worker — runs: transcript (**once** — BullMQ owns retrying) → all four `ContentType`s generated **in parallel** via `Promise.all` → one Prisma `$transaction` that deletes prior content, inserts the new rows, and flips the job to `COMPLETED`. It is idempotent: it skips jobs already `COMPLETED` or missing from the DB.

   **Retries belong to BullMQ, and only the last one may write `FAILED`.** There used to be a `for (attempt 1..3)` loop inside `process()`, nested inside BullMQ's own 3 attempts — nine transcriptions and nine audio downloads for one job. And `FAILED` was written on every attempt, including 1 and 2 of 3; since the SSE stream completes on `FAILED`, the dashboard showed a terminal failure while the job was still retrying, then quietly succeeded behind a card that said Failed. Measured against BullMQ 5: inside `process()` `attemptsMade` is **0-based** (0, 1, 2) and has only been incremented by the time the `failed` event fires, so the run in progress is `attemptsMade + 1`. `jobs.processor.spec.ts` pins all of this.

   The processor also owns the downloaded audio's lifetime outright — it keeps the temp file between attempts so a retry reuses it, and deletes it on success or on the final failure. That used to be split across `TranscriptionService`'s `finally` and the processor, with two different ideas of "last attempt".

   **A `FAILED` job carries a reason** in `Job.error`, mapped by [job-error.ts](ai-repurposer-backend/src/modules/jobs/job-error.ts). That mapper never returns text taken from an exception — yt-dlp echoes its whole command line including the temp path, Prisma errors carry the connection string, and the column is served over the API. `safeJobError` re-checks the value against the table on the way to the database, so a future call site cannot smuggle a raw message in.
...
- **Plan quota** — Redis key `usage:{userId}:{YYYY-MM}` (see [plans.config.ts](ai-repurposer-backend/src/common/config/plans.config.ts)), expiring at the start of next month UTC. `FREE` = 1 job/month, `PRO` = `Infinity`. Claimed by `UsageService.tryConsume` from inside `JobsService`, reported by `GET /users/me`. Redis is the only store, so quota resets if Redis is flushed — the `User.jobsUsedThisMonth`/`usagePeriodStart` columns that used to suggest otherwise were dropped, since nothing read or wrote them. **Both the key and its TTL are built in UTC**; the key used to use local time while the expiry used UTC, so away from UTC there was a window of up to a day where the month rolled over on one and not the other.
